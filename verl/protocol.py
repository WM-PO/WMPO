...
class DataProto:
    ...
    def pop_batch(self, pop_size, dim=0):
        current_len = len(self)
        if pop_size > current_len:
            raise ValueError(f"Cannot pop {pop_size} samples from batch of size {current_len}")

        # slice out the front pop_size samples
        popped = self.slice(slice(0, pop_size))

        remaining_size = current_len - pop_size
        if remaining_size == 0:
            self.batch = TensorDict({}, batch_size=[0])
        else:
            # 构造一个新的 TensorDict，所有 value 都是裁剪后的 narrow
            new_batch = {
                key: val.narrow(dim, start=pop_size, length=remaining_size)
                for key, val in self.batch.items()
            }
            self.batch = TensorDict(new_batch, batch_size=[remaining_size])

        # 同理处理 non_tensor_batch
        for key, val in self.non_tensor_batch.items():
            self.non_tensor_batch[key] = val[pop_size:]
