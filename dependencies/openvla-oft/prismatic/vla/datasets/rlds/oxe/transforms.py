...
def square_d0_dataset_transform(trajectory):
    # 取前 6 维 + 夹爪
    actions = tf.convert_to_tensor(trajectory["action"], dtype=tf.float32)
    # gripper = actions[:, -1:]
    # gripper = invert_gripper_actions(tf.clip_by_value(gripper, 0, 1))

    # trajectory["action"] = tf.concat([actions[:, :6], gripper], axis=1)

    # 如果有 state，就添加派生字段
    # if "state" in trajectory["observation"]:
    #     state = tf.convert_to_tensor(trajectory["observation"]["state"], dtype=tf.float32)
    #     trajectory["observation"]["EEF_state"] = state[:, :6]
    #     trajectory["observation"]["gripper_state"] = state[:, -2:]
    return trajectory


def mimicgen_dataset_transform(trajectory):
    return trajectory
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    ### robomimic
    "coffee_d0_300_demos": mimicgen_dataset_transform, 
    "stack_three_d0_300_demos": mimicgen_dataset_transform, 
    "three_piece_assembly_d0_300_demos": mimicgen_dataset_transform, 
    "square_d0_300_demos": mimicgen_dataset_transform, 
    # "square_d0_300_demos": square_d0_dataset_transform,
    # "square_d0_1000_demos": square_d0_dataset_transform,
    "aloha_adjust_bottle_tfds": aloha_dataset_transform,
